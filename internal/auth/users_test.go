...
func TestGetDefaultUserPath(t *testing.T) {
	t.Run("returns correct default path", func(t *testing.T) {
		...
		expectedPath := "/path/to/binary/postgres-mcp-users.yaml"
	...
	t.Run("handles relative paths", func(t *testing.T) {
		...
		if path != "bin/postgres-mcp-users.yaml" {
			t.Errorf("Expected relative path 'bin/postgres-mcp-users.yaml', got '%s'", path)
		}
	})
}
