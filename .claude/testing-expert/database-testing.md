# Database Testing - pgEdge Postgres MCP Server

This document describes how to test database-dependent code in the
Postgres MCP Server project.

## Overview

Database testing in the Postgres MCP Server involves:
...
**Database Naming**: `pgedge_mcp_test_<unix_timestamp>`

Example: `pgedge_mcp_test_1699564823`
...
**Usage**:
```bash
export TEST_PGEDGE_MCP_SERVER=postgres://user:password@hostname:5432/postgres
go test ./...
```
...
func skipIfNoDatabase(t *testing.T) *pgxpool.Pool {
    ...
    if connStr == "" {
        t.Skip("Skipping database test (TEST_PGEDGE_MCP_SERVER not set)")
    }
...
```bash
# Run tests with TEST_PGEDGE_MCP_KEEP_DB
TEST_PGEDGE_MCP_KEEP_DB=1 go test -v ./...

# Output will show database name:
# Keeping test database: pgedge_mcp_test_1699564823

# Connect to it
psql postgres://postgres@localhost:5432/pgedge_mcp_test_1699564823
