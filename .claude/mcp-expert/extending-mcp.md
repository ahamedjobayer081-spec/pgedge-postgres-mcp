...
Add your tool to the privilege system in `/internal/auth/privileges.go`:
...
Add your tool definition to `/internal/mcp/server.go` in the `handleListTools()` function:
...
Create the handler function in `/internal/mcp/server.go`:
...
```go
// In a separate package like /internal/tools/export.go
package schema
...
Create tests in `/internal/mcp/server_test.go`:
...
Create integration tests in `/internal/`:
...
Add your tool to `/.claude/mcp-expert/tools-catalog.md`:
...
1. **URI scheme** - Follow `pg://<collection>` pattern
...
In `/internal/mcp/server.go`, add to `handleListResources()`:
    ...
    case "pg://connections":
        ...
        for rows.Next() {
            ...
            contents = append(contents, map[string]interface{}{
                "uri":      fmt.Sprintf("pg://connections/%d", id),
                "mimeType": "application/json",
...
Add to `/.claude/mcp-expert/resources-catalog.md`.
