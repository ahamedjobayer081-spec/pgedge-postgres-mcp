...
This is the authoritative style guide for all documentation in the pgEdge
Postgres MCP Server project. These rules are derived from CLAUDE.md and must be
...
```markdown
<!-- GOOD -->
The server stores connection data in PostgreSQL; it manages them for
MCP clients.

The server handles authentication; the client manages user sessions;
the tools execute database operations.
```
...
```markdown
See the [schema definition](https://github.com/pgEdge/pgedge-postgres-mcp/
blob/main/internal/database/schema.go).
```
