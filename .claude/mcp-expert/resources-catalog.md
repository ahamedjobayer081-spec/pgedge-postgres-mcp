...
Resources in MCP provide read-only access to server-side data. They use a URI-based addressing scheme with the custom `pg://` protocol.
...
```json
{
    "contents": [
        {
            "uri": "pg://service-tokens/ci-token",
            "mimeType": "application/json",
...
```
pg://<collection>[/<item-id>]
```

**Examples:**

- `pg://users` - Collection of all users
- `pg://users/alice` - Specific user (returned in contents)
- `pg://service-tokens` - Collection of all service tokens
- `pg://service-tokens/ci-token` - Specific token (returned in contents)
...
for rows.Next() {
    ...
    contents = append(contents, map[string]interface{}{
        "uri":      fmt.Sprintf("pg://users/%s", username),
        "mimeType": "application/json",
...
case "pg://groups":
    ...
    for rows.Next() {
        ...
        contents = append(contents, map[string]interface{}{
            "uri":      fmt.Sprintf("pg://groups/%d", id),
            "mimeType": "application/json",
