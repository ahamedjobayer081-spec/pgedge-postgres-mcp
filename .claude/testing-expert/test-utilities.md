# Test Utilities Reference - pgEdge Postgres MCP Server

This document provides detailed information about available test utilities
and how to use them.

## Overview

Test utilities are located in `/tests/testutil/` and provide common
functionality for integration testing across the Postgres MCP Server
components.
...
type TestDatabase struct {
    Name          string         // Database name (pgedge_mcp_test_<timestamp>)
...
**Behavior**:
1. Generates unique database name: `pgedge_mcp_test_<unix_timestamp>`
2. Connects to PostgreSQL using `TEST_PGEDGE_MCP_SERVER` or default
3. Creates the test database
4. Returns connection pool to the test database

**Environment Variables**:
- `TEST_PGEDGE_MCP_SERVER`: PostgreSQL connection (default: `postgres://postgres@localhost:5432/postgres`)
- `TEST_PGEDGE_MCP_KEEP_DB=1`: Keep database after tests
...
2. If `TEST_PGEDGE_MCP_KEEP_DB=1`, prints database name and returns
3. Otherwise:
...
**Behavior**:
1. Finds server binary in `../cmd/pgedge-pg-mcp-svr/`
...
**Behavior**:
1. Finds CLI binary in `../cmd/pgedge-pg-mcp-cli/`
...
3. Generates unique config filename: `test-pgedge_mcp_test_<timestamp>.conf`
...
```
Error: MCP server binary not found at ../cmd/pgedge-pg-mcp-svr/pgedge-pg-mcp-svr
```
...
```bash
export TEST_PGEDGE_MCP_SERVER=postgres://postgres@localhost:5432/postgres
```
