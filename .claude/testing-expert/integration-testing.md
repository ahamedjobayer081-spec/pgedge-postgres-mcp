# Integration Testing - pgEdge Postgres MCP Server

This document describes the integration testing structure and approach for the
Postgres MCP Server project.

## Overview

Integration tests verify that multiple components work together correctly.
The Postgres MCP Server has two levels of integration testing:
...
func TestUserOperations(t *testing.T) {
    ...
    // Database name: pgedge_mcp_test_<timestamp>
...
- `TEST_PGEDGE_MCP_SERVER`: PostgreSQL connection string (default: `postgres://postgres@localhost:5432/postgres`)
- `TEST_PGEDGE_MCP_KEEP_DB=1`: Keep database after tests for inspection
...
**Location**: `/internal/integration/` or co-located in packages
...
func TestPrivilegesIntegration(t *testing.T) {
    ...
    connStr := os.Getenv("TEST_PGEDGE_MCP_SERVER")
    if connStr == "" {
        t.Skip("TEST_PGEDGE_MCP_SERVER not set")
    }
...
**Location**: Co-located in packages (e.g., `/internal/database/`)
...
TEST_PGEDGE_MCP_SERVER=postgres://user:pass@host/db \
TEST_PGEDGE_MCP_KEEP_DB=1 \
go test -v ./integration/...
```

### Environment Variables

**TEST_PGEDGE_MCP_SERVER**: Database connection string
```bash
export TEST_PGEDGE_MCP_SERVER=postgres://postgres:password@localhost:5432/postgres
```

**TEST_PGEDGE_MCP_KEEP_DB**: Keep test database after tests
```bash
export TEST_PGEDGE_MCP_KEEP_DB=1
```
...
```bash
TEST_PGEDGE_MCP_KEEP_DB=1 make test

# Connect to test database
psql postgres://postgres@localhost:5432/pgedge_mcp_test_<timestamp>
...
- Check TEST_PGEDGE_MCP_SERVER variable
