...
This document describes how the Model Context Protocol (MCP) is implemented in the pgEdge Postgres MCP Server.
...
- `/internal/mcp/types.go` - Core protocol types and constructors
- `/internal/mcp/server.go` - MCP server and method handlers
- `/internal/mcp/http_server.go` - HTTP/HTTPS server with SSE support
- `/cmd/pgedge-pg-mcp-svr/main.go` - Application entry point
