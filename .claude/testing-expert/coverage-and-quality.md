# Coverage and Code Quality - pgEdge Postgres MCP Server

This document describes code coverage measurement and quality checks for the
...
Code coverage measures which parts of the codebase are exercised by tests. The Postgres MCP Server uses Go's built-in coverage tools for GoLang projects and Jest for React projects.
...
# Output example:
# pgedge-postgres-mcp/database/datastore.go:45:    NewDatastore        100.0%
# pgedge-postgres-mcp/database/datastore.go:67:    Close               100.0%
# pgedge-postgres-mcp/database/datastore.go:89:    GetConnection       87.5%
```
...
```
mode: set
pgedge-postgres-mcp/database/datastore.go:45.41,47.16 2 1
pgedge-postgres-mcp/database/datastore.go:47.16,49.3 1 1
pgedge-postgres-mcp/database/datastore.go:50.2,50.26 1 1
```
...
go test -coverprofile=cmd.out ./cmd/...
go test -coverprofile=tests.out ./tests/...

# Merge coverage files
echo "mode: set" > coverage.out
tail -q -n +2 internal.out cmd.out tests.out >> coverage.out
...
# Output:
# ok      pgedge-postgres-mcp/database    0.123s  coverage: 87.5% of statements
```
